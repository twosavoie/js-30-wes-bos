...
1: drumkit, keydown, play a sound
2: clock, transitions, get... seconds, hours, minutes from Date()
3: sliders to update styling, setProperty, change, mousemove
4: array methods map, filter, reduce, sort
5: toggle transitions, methods toggle and includes, el click, transitionend
6: filter for city or state, methods fetch, then, filter, match, RegExp, replace, returning HTML
7: array methods some, every, find, findIndex, slice
8: drawing canvas (it's pretty!) methods: offsetX and Y, linewidth, el mouseys
9: using the console
10: checkboxes select all and loop over with forEach, click first and last to check off a range of checkboxes
11: video player, methods paused, currentTime, parseFloat, duration, offsetWidth, lots of el
12: detect key sequence - unicorns appear, methods push, splice
13: images slide in when page is scrolled, debounce, scrollY, innerHeight, offsetTop
14: difference of a copy vs a reference methods: apply, JSON parse & stringify, assign, spread, concat, slice
15: add items to a list and check them off (checkoff is an emoji), local storage, event target
16: text shadow follows cursor, figuring offsets with mousemove
17: array methods to sort an array strip, sort, map
18: add up the time for a series of videos, working with seconds
19: using the camera for a video feed and applying filters
20: speech detection to create a transcript, interimResults, map, appendChild, includes
21: geolocation (no longer works on Mac the same) methods watchPosition, coords.speed, coords.heading
22: follow mousemove to add a highlight behind links, methods: getBoundingClientRect, .top, .left, .width, .height, .transform, window.scrollY
23: speech synthesis changing voice from OS, rate, and pitch as it reads to a user, starting and stopping, methods: getVoices, map, filter, join, cancel, value, voiceschanged
24: sticky nav with slideout logo, window position methods: paddingTop, scrollY, offsetHeight, el: scroll
25: how an event listener bubbles up, capture, once
26: Stripe follow along nav places resizing bkgrd behind text on hover, methods: getBoundingClientRect, dropdownCoords.height, dropdownCoords.top, setProperty
27: click and drag elements, methods: pageX, offsetLeft
28: video speed controller, control slider, methods: pageY, Math.round
29: countdown timer - really like this one, methods: Date.now(), title, textContent, getHours, getMinutes, parseInt, reset (resetting a form)
30: game to whack a mole, methods: setTimeout, Math.round, length, (!e.isTrusted)
